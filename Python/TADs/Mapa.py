...
#pos=[n° fila,n° columna], donde arrancamos a contar desde 0.
    ...
    def alcanzar_con_mano_derecha(self,pos_inicial,pos_final):
        self.map=open(self.map.name,'r')
        filas_mapa=self.map.readlines()
        bool_final=True
        tmp=pos_inicial
        while(filas_mapa[tmp[0]+1][tmp[1]]==1):
            if(filas_mapa[tmp[0]][tmp[1]+1]==0):
                tmp[1]=tmp[1]+1
            elif(filas_mapa[tmp[0]-1][tmp[1]]==0):
                tmp[0]=tmp[0]-1
            else:
                bool_final=False
...
#Idea alcanzar_mano_derecha: dado pos_inicial, si abajo tengo 1 o termina el archivo entonces:
            #si hay un 0: doy un paso hacia la derecha, e itero hasta que abajo no tenga 1.    
