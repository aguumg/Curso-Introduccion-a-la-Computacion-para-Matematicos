...
def es_rectangulo(file):                                                        #Devuelve un bool segun el archivo sea un rectangulo o no.
    file=open(sys.argv[2],'r')
    filas_archivo=file.readlines()
    longitud=len(quitar_barra_n(list(filas_archivo[0])))                        #Le saco \n a la primera fila del archivo, pero como file.readlines[0] lo lee como un solo string, primero tengo que convertirlo a una lista de strings.
    i=0
    bool_final=True
    while(i<len(filas_archivo)):                                                #Esta secuencia hace las veces del for line in file (la uso varias veces durante el programa).
...
def cuadrado_de_unos(file, nro_fila, indice):            #En list[indice] hay un 0, la funcion recorre "el cuadrado alrededor de este" y se fija que sean todos unos. Devuelve bool. Es decir, recorremos las filas nro_fila-1 y nro_fila+1, y nos fijamos que tengan unos en las posiciones indice-1 indice e indice+1. Y nos fijamos que la fila nro_fila tenga unos en las posiciones indice-1 e indice+1.
        file=open(sys.argv[2],'r')
        filas_archivo=file.readlines()
        fila_del_cero_con_unos=(filas_archivo[nro_fila][indice-1]=='1') and (filas_archivo[nro_fila][indice+1]=='1')                  #La funcion fila_k devuelve una lista de strings, por lo que tengo que buscar '1'.
        fila_superior_con_unos=detector_unos(filas_archivo[nro_fila-1][indice-1:indice+2])                                          #Notar que no hay posibilidad de que nro_fila-1, indice-1<0 porque tuvimos la cautela de separar estos casos en la funcion espacios_rodeados.
...
def espacios_rodeados(file):                                    #La idea es la siguiente: recorremos las listas del archivo, arrancando de la 2da y hasta la anteultima, y desde la segunda posicion a la ultima. Cuando detecttamos un 0, llamamos la funcion cuadrado_de_unos.
        ...
        filas_archivo=file.readlines()
        rodeados=0
        nro_fila=1                                          #Arranco desde las 2da fila, o sea filas_archivo[1].
        while(nro_fila<len(filas_archivo)-1):                   #Buscamos un 0 encerrado, por lo que arrancamos desde la 2da fila y terminamos en la anteultima.
...
def dimensiones(file):                                          #Devuelve dim=[#filas,#columnas].
        file=open(sys.argv[2],'r')
        filas_archivo=file.readlines()
        dim=[len(filas_archivo),len(quitar_barra_n(list(filas_archivo[0])))]                    #Como solo tiene sentido hablar de columnas para un buen mapa, la cantidad de columnas es la cantidad de elementos de cualquiera de sus lineas
        file.close()
        return dim
...
if(es_mapa(sys.argv[2])==True):
        if (sys.argv[1]=='es_mapa' or sys.argv[1]=='cantidad_paredes' or sys.argv[1]=='espacios_rodeados' or sys.argv[1]=='dimensiones' or sys.argv[1]=='corredor_horizontal_mas_largo' or sys.argv[1]=='densidad'):
            ...
            if(sys.argv[1]=='espacios_rodeados'):
                    print('El mapa tiene ' + str(espacios_rodeados(open(sys.argv[2],'r'))) + ' huecos rodeados de paredes.')
            if(sys.argv[1]=='dimensiones'):
                    print('El mapa tiene ' + str(dimensiones(open(sys.argv[2],'r'))[1]) + ' cuadrados de ancho y ' + str(dimensiones(open(sys.argv[2],'r'))[0]) + ' de alto.')
            if(sys.argv[1]=='corredor_horizontal_mas_largo'):
                    print('El corredor horizontal más largo tiene longitud ' + str(corredor_horizontal_mas_largo(open(sys.argv[2],'r'))) + '.')
            if(sys.argv[1]=='densidad'):
                    print('La densidad arquitectónica es de ' + str(densidad(open(sys.argv[2],'r'))) + '.')
        else:
